...
// Destrutor da Lista (libera a memória de todas as células e games)
void destruirLista(ListaGames* lista) {
    Celula* i = lista->primeiro;
    while (i != NULL) {
        Celula* tmp = i;
        if (tmp->elemento != NULL) {
            free(tmp->elemento); // Libera a memória do Game
        }
        i = i->prox;
        free(tmp); // Libera a célula
    }
...
void splitCsvAspas(char *linha, char campos[][MAX_ARRAY_STRING]) {
    ...
    while (linha[i] != '\0' && j < 14) {
        ...
        campos[j][k] = '\0';
        // Limpar espaços e chars especiais após o parse (importante)
...
/**
 * Imprime um Game no formato correto.
 */
...
int main() {
    ...
    // Lendo IDs até "FIM" ou até a lista de IDs acabar
    while (scanf("%d", &id) == 1) {
        if (id > 0) {
    ...
    for (int i = 0; i < numOperacoes; i++) {
        ...
        if (sscanf(linhaOperacao, "%4s %d %d", operacao, &posOp, &idOp) == 3) {
            ...
            // Operação II, IF, R*
            idOp = idOp; // Pode ser ID para I ou POS para R*
            posOp = -1; // Não usado
    ...
    // Limpeza de memória (Libera apenas as células, não os Games, que serão liberados no final)
    // O array 'todosOsGames' contém todos os ponteiros de 'Game', que devem ser liberados uma vez.
    for(int i = 0; i < totalGames; i++) {
        free(todosOsGames[i]);
    }
    free(todosOsGames);
    
    // Libera as células e o sentinela. Os jogos já foram liberados acima, então a Celula só libera a si mesma.
    destruirLista(&lista);
