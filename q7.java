...
public class q7 {
    ...
    /*rRetorna o restaurante do indice i formatado conforme especificacao */
    ...
    /*
     * intercala subIdx[inicio..meio] e subIdx[meio+1..fim]
     * criterio: cidade, desempate por nome
     */
    static void merge(int inicio, int meio, int fim) {
        ...
        while (i <= meio && j <= fim) {
            comp++;
            int cmp = cidade[subIdx[i]].compareTo(cidade[subIdx[j]]);
            if (cmp == 0) {
                comp++;
                cmp = nome[subIdx[i]].compareTo(nome[subIdx[j]]);
            }
            if (cmp <= 0) { aux[k++] = subIdx[i++]; }
            else          { aux[k++] = subIdx[j++]; }
            mov++;
        }
        while (i <= meio) { aux[k++] = subIdx[i++]; mov++; }
        while (j <= fim)  { aux[k++] = subIdx[j++]; mov++; }
        for (int x = 0; x < tamanho; x++) {
            subIdx[inicio + x] = aux[x];
            mov++;
        }
    }

    /*
     * divide subIdx recursivamente e chama merge
     * Ordena por cidade, desempate por nome
     */
    static void mergesort(int inicio, int fim) {
        if (inicio < fim) {
     ...
     * desempate nome via mergesort e gera arquivo de log
     */
    public static void main(String[] args) {
        ...
        try {
            PrintWriter log = new PrintWriter("matricula_mergesort.txt");
