...
public class HashReserva {
    ...
    static class TabelaHash {
        ...
        public void inserir(Game g) {
            ...
            if (tabela[pos] == null) {
                ...
                // Colisão -> Área de reserva
                if (reservaAtual < (m1 + m2)) {
                    tabela[reservaAtual] = g;
                    reservaAtual++;
                } 
            }
        }
        ...
        public int pesquisar(String nome) {
            ...
            // Varre a reserva
            for (int i = m1; i < reservaAtual; i++) {
                if (tabela[i] != null) {
                    comparacoes++;
                    if (tabela[i].name.equals(nome)) {
                        return i;
                    }
                }
            }
    ...
    public static void main(String[] args) throws Exception {
        ...
        while (true) {
            ...
            int posHash = tabela.hash(nomeLimpo);

            if (posEncontrada != -1) {
                
                System.out.println(nomeLimpo + ": (Posicao: " + posEncontrada + ") SIM");
            } else {
                
                System.out.println(nomeLimpo + ": (Posicao: " + posHash + ") NAO");
            }
        }
